...
class OpenTestPage extends TestPage {
  ...
  OpenTestPage({Key? key}) : super('Open tests', key: key) {
    ...
    test('Open onUpgrade', () async {
      ...
      try {
        expect(await database.insert('Test', <String, Object?>{'id': 1, 'name': 'test'}), 1);
      } finally {
        await database.close();
      }
    });
    ...
    test('Open onDowngrade', () async {
      ...
      database = await openDatabase(path, version: 1, onDowngrade: (Database db, int oldVersion, int newVersion) async {
        expect(oldVersion, 2);
        expect(newVersion, 1);
        await db.execute('ALTER TABLE Test ADD name TEXT');
        onDowngrade = true;
    ...
    test('Open bad path', () async {
      try {
        ...
        final db = await openDatabase('my_path/my_db.db');
        print('db的值:$db');
        // fail();
      } on DatabaseException catch (e) {
    ...
    test('Open onDowngrade delete', () async {
      ...
      var database = await openDatabase(path, version: 3, onCreate: (Database db, int version) async {
        await db.execute('CREATE TABLE Test(id INTEGER PRIMARY KEY)');
      });
      await database.close();
      ...
      var onConfigured = false;
      database = await openDatabase(path, version: 2, onConfigure: (Database db) {
      ...
      database = await openDatabase(path, version: 2, onCreate: (Database db, int version) {
    ...
    test('Open batch', () async {
      ...
      final db = await openDatabase(path, version: 1, onConfigure: onConfigure, onCreate: onCreate, onOpen: onOpen);
    ...
    test('Open read-only', () async {
      ...
      db = await openReadOnlyDatabase(path);
      expect(Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM Test')), 1);
      //这里有问题，要用到readonly接口才能解决 ,使用了readonly接口，也能进行插入操作
      try {
    ...
    test('Open demo (doc)', () async {
...

      {
        ...
        final db = await openDatabase(path,
            version: 1, onCreate: onCreate, onUpgrade: onUpgrade, onDowngrade: onDatabaseDowngradeDelete);
        await db.close();
      }
    ...
    test('single/multi instance (using factory)', () async {
      ...
      try {
        db1 = await databaseFactory.openDatabase(path, options: OpenDatabaseOptions(singleInstance: false));
        db2 = await databaseFactory.openDatabase(path, options: OpenDatabaseOptions(singleInstance: true));
        db3 = await databaseFactory.openDatabase(path, options: OpenDatabaseOptions(singleInstance: true));
    ...
    test('open in sub sub directory', () async {
      // await Sqflite.devSetDebugModeOn(true);
      final databasesPath = await factory.getDatabasesPath();
      final path = join(databasesPath, 'sub2_that_should_not_exists', 'sub_sub');
      try {
        await deleteDirectory(path);
      } catch (_) {}
    ...
    test('open_close_open_no_wait', () async {
      ...
      final factory = databaseFactory;
      await factory.deleteDatabase(path);
      final db = await factory.openDatabase(path, options: OpenDatabaseOptions(version: 1));
      try {
        expect(await db.getVersion(), 1);
        // close no wait
        unawaited(db.close());
        final db2 = await factory.openDatabase(path, options: OpenDatabaseOptions(version: 1));
        print('$db, $db2');
        verify(db != db2);
    ...
    test('close in transaction', () async {
      ...
      final factory = databaseFactory;
      await factory.deleteDatabase(path);
      var db = await factory.openDatabase(path, options: OpenDatabaseOptions(version: 1));
      try {
        //await db.getVersion();
        await db.execute('BEGIN IMMEDIATE');
        await db.close();

        db = await factory.openDatabase(path, options: OpenDatabaseOptions(version: 1));
      } finally {
        await db.close();
      }
    });
    ...
    test('Open non sqlite file', () async {
      ...
      // await Sqflite.devSetDebugModeOn(true);
      final factory = databaseFactory;
      final path = join(await factory.getDatabasesPath(), 'test_non_sqlite_file.db');
      ...
      // try read-only
      {
        ...
        try {
          //这里有问题,14800030
          db = await factory.openDatabase(path, options: OpenDatabaseOptions(readOnly: true));
        } catch (e) {
          print('open error');
        }
        try {
      ...
      var versionShouldFail = !supportsCompatMode || platform.isIOS || platform.isMacOS;
      ...
      if (platform.isAndroid) {
        ...
        expect((await readFileAsBytes(path)).length, greaterThan(minExpectedSize));
      }
    });
  }
}
